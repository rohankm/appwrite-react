...
import type { StorageFileOperation } from "../storage/types";
...
export function useBucket(
  ...
  const queryKey = useMemo(
    () => ["appwrite", "storage", bucketId, { queries }],
    [bucketId, queries]
  );
    ...
    onSuccess: (files) => {
      for (const file of files) {
        queryClient.setQueryData(
          ["appwrite", "storage", bucketId, file.$id],
          file
        );
      }
    },
  ...
  useEffect(() => {
    const unsubscribe = storage.client.subscribe(
